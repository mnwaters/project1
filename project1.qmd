...
summary.census <- function(
    ...
    for (i in 1:length(categoricalVar)) {
      tibble_list <- list()
      tibble_list[i] <- classcensus |>
        group_by(PWGTP) |>
        select(-colnames(numericalVar)) |>
        pivot_wider(names_from = categoricalVar[i], values_from = categoricalVar[i], values_fn = length, values_fill = 0) |>
        arrange(PWGTP)
      if (i == 1) {
        acsSummaryCat <- tibble_list[1]
      } else {
        acsSummaryCat <- full_join(tibble_list[i], tibble_list[i-1])
      }
    }
